...
class DatasetList(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return sample.type(torch.FloatTensor), target#.astype(np.float32)
...
class ImageRelLists(DatasetList):
    ...
    """
    def __init__(self, image_paths, image_root=None, transform=None, target_transform=None,
        ...
        for s_item in image_paths:
            if image_root != None:
                ...
                image_full_path = s_item[0]
            samples.append((image_full_path.replace('\\', '/'), s_item[1]))
...
class ImageIdexinDicts(DatasetList):
    def __init__(self, image_ids, annotation_dicts, image_root, transform=None, target_transform=None,
        ...
        super(ImageIdexinDicts, self).__init__(samples, loader,
                                               transform=transform,
                                               target_transform=target_transform)
...
class ImageDict(DatasetList):
    def __init__(self, annotation_dicts, image_root, transform=None, target_transform=None,
                 loader=default_loader):

        samples = []
        for s_image_id in annotation_dicts:
            s_information = annotation_dicts[s_image_id]
            image_full_path = os.path.join(image_root, s_information[0])
            samples.append((image_full_path, s_information[1:]))
        ...
        super(ImageDict, self).__init__(samples, loader,
                                               transform=transform,
                                               target_transform=target_transform)
        self.imgs = self.samples