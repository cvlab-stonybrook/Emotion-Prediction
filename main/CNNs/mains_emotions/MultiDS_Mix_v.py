...
def main():
    ...
    for k in range(num_datasets):
        ...
        if not args.evaluate: #else:
            ...
            train_loaders[args.ind] = torch.utils.data.DataLoader(
                ...
                num_workers=args.workers, pin_memory=False, sampler=train_samplers, collate_fn=none_collate)
    setattr(args, 'num_iter', num_iter)

    # TRAINING
    best_score = [-1 for _ in range(num_datasets)]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loads_iters, train_loaders, model, criterion, optimizer, epoch, args, print_func)

        # evaluate and save
        val_score = [None for _ in range(num_datasets)]
        test_score = [None for _ in range(num_datasets)]
        for j in range(num_datasets):
            # evaluate on validation set
            if val_loaders[j]:
                val_score[j] = validate(val_loaders[j], model, criterion, args, print_func, j)
            else:
                val_score[j] = 0
            # remember best score and save checkpoint
            is_best[j] = val_score[j] > best_score[j]
            best_score[j] = max(val_score[j], best_score[j])
            ...
            CNN_utils.save_checkpoint({
                ...
                'best_score': best_score[j],
                'optimizer': optimizer.state_dict(),
            }, is_best[j], file_directory=save_directory, epoch=epoch, save_best_only=args.save_best_only, ind=save_ind)

            test_score[j] = validate(test_loaders[j], model, criterion, args, print_func, j, phase='Test')

        print_func("Val scores: {}".format(val_score))
        print_func("Test scores: {}".format(test_score))
...
def validate(val_loader, model, criterion, args, print_func, ind, phase='Validation'):
    ...
    with torch.no_grad():
        ...
        print_func('{phase} * Score {top1.avg:.3f}'
              .format(phase=phase, top1=losses))
