...
def train(train_loads_iter, train_loaders, model, criterion, optimizer, epoch, args, print_func):
    ...
    for i in range(batch_iters):
        ...
        for ds in range(args.num_datasets):
            ...
            dom_target = torch.tensor(np.array([args.ind for _ in range(list(target.size())[0])]), dtype=torch.float).cuda(args.gpu, non_blocking=True)
            dtarget = dom_target.cuda(args.gpu, non_blocking=True)
            domain_loss = F.binary_cross_entropy_with_logits(dom_target, dtarget)
        ...
        if i % args.print_freq == 0:
            print_func('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec@X {topX.val:.3f} ({topX.avg:.3f})'.format(
                   ...
                   data_time=data_time, loss=losses, top1=top1, topX=topX))
...
def validate(val_loader, model, criterion, args, print_func, ind, phase='Validation'):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # compute output
            output = model(input)
            output = output[(args.class_len[ind] // 2) % 3]
