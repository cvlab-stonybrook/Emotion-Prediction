...
def save_checkpoint(state, is_best, save_best_only=False, file_directory=None, epoch=0, ind='#'):
    if ind == '#':
        filename = 'checkpoint_{:04d}.pth.tar'
    else:
        filename = 'checkpoint_{:04d}_{:01d}.pth.tar'
    ...
    if ind == '':
        ...
        file_path = os.path.join(file_directory, filename.format(epoch, ind))
    ...
    if is_best:
        if ind == '#':
            ...
            model_best_path = os.path.join(file_directory, 'model_best_' + str(ind) + '.pth.tar')
        if save_best_only:
            shutil.move(file_path, model_best_path)
        else:
...
def freeze_all_except_fc(model):
    def dfs_freeze(model):
        for name, child in model.named_children():
            ...
            dfs_freeze(child)
    dfs_freeze(model)
    if hasattr(model, 'fc_0'):
        fc_list = [i for i in dir(model) if i.startswith('fc_')]
        for fc_name in fc_list:
            getattr(model, fc_name).weight.requires_grad = True
            getattr(model, fc_name).bias.requires_grad = True
